...
# source ./generators/gitlab_ci_yaml.sh
...
prepareRepo() {
    ...
    dotnet nuget remove source 'App Nugets'
...
generateBogus() {
    ...
    # Contract Bogus Generators
    cd $target_dir/tests/bogus
    dotnet new classlib -n $1.Contract.Bogus
    cd $target_dir/tests/bogus/$1.Contract.Bogus
    rm -rf Class1.cs
    dotnet add package $bogus_sdk  -v $sdk_version
    dotnet add reference $target_dir/src/internal/$1.Contract
    dotnet add reference $target_dir/tests/bogus/$1.Schema.Bogus
}
...
generateSchema() {
    mkdir -p $target_dir/src/internal
    cd $target_dir/src/internal
    ...
    cd $target_dir/tests/unit
    generateTest $1.Schema.UnitTests
    cd $target_dir/tests/unit/$1.Schema.UnitTests
    dotnet add reference $target_dir/src/internal/$1.Schema
    dotnet add reference $target_dir/tests/bogus/$1.Schema.Bogus
}
...
generateContract() {
    ...
    cd $target_dir/tests/unit/$1.Contract.UnitTests
    dotnet add reference $target_dir/src/internal/$1.Contract
    dotnet add reference $target_dir/tests/bogus/$1.Contract.Bogus
}
...
generateDomain() {
    ...
    cd $target_dir/src/internal/$1.Domain
    rm -rf Class1.cs
...
WORKDIR /src
COPY ["src/hosts/$1/$1.csproj", "$1/"]
RUN dotnet restore "src/hosts/$1/$1.csproj"
COPY . .
WORKDIR "/src/$1"
RUN dotnet build "$1.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "$1.csproj" -c Release -o /app/publish --runtime alpine-x64 --self-contained 
